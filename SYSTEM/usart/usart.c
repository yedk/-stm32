...
u8 USART_RX_BUF2[USART_REC_LEN];     //uart2接受缓冲；
u8 voice_flag;
//接收状态
//bit15，	接收完成标志
//bit14，	接收到0x0d
...
u16 USART_RX_STA_2=0;     //接收状态标记2
u8 USART3_RX_STA =0;      //语音合成标志
	...
	if(USART_GetITStatus(USART1, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if((USART_RX_STA&0x8000)==0)//接收未完成
			...
			if(USART_RX_STA&0x4000)//接收到了0x0d
				...
				if(Res!=0x0f)
				{
					USART_RX_STA=USART_RX_STA&0xBFFF;//USART_RX_STA=0;//接收错误,重新开始
...
void USART2_IRQHandler(void)                	//串口2中断服务程序
	...
	if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)  //接收中断(接收到的数据必须是0x0d 0x0a结尾)
		...
		if(USART_RX_STA_2<13)
		{
			USART_RX_BUF2[USART_RX_STA_2++]=Res;
		}
		/*if((USART_RX_STA_2&0x8000)==0)//接收未完成
			{
			if(USART_RX_STA_2&0x4000)//接收到了0x0d
				{
				if(Res!=0x0a)USART_RX_STA_2=0;//接收错误,重新开始
				...
				if(Res==0x0d)USART_RX_STA_2|=0x4000;
					...
					{
					USART_RX_BUF2[USART_RX_STA_2&0X3FFF]=Res ;
					USART_RX_STA_2++;
